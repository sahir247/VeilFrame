    ...
    def get_settings(self) -> ProcessingSettings:
        ...
        return ProcessingSettings(
            ...
            quality_gate=VisualBudgetPolicy(
                ...
                policy_budget=0.10 if "10%" in self.combo_presets.currentText() else 0.05,
                aggregate_ceiling_pct=10.0 if "10%" in self.combo_presets.currentText() else 5.0,
                spatial_ceiling_pct=4.0 if "10%" in self.combo_presets.currentText() else 2.0,
                temporal_ceiling_pct=2.0 if "10%" in self.combo_presets.currentText() else 1.0,
                luma_ceiling_pct=2.0 if "10%" in self.combo_presets.currentText() else 1.0,
                chroma_ceiling_pct=2.0 if "10%" in self.combo_presets.currentText() else 1.0,
                frequency_ceiling_pct=2.0 if "10%" in self.combo_presets.currentText() else 1.0,
