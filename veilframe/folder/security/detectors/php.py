"""
veilframe.folder.security.detectors.php — PHP credential detector.
"""
...
PHP_ASSIGNMENT_REGEX = re.compile(
    r"""(?im)^(?P<prefix>[ \t]*(?:public|private|protected|static)?\s*\$(?P<var>[a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*)(?P<quote>["'])(?P<token>[^"'\r\n]{20,})(?P=quote)(?P<semi>;?)"""
)


class PHPSecretDetector(BaseSecretDetector):
    """Detects credentials in PHP source files."""
    ...
    def detect(self, content: str, file_path: str = "") -> List[SecretFinding]:
        ...
        for idx, line in enumerate(content.splitlines(), 1):
            ...
            if not line_str or line_str.startswith(("//", "#", "/*")):
                continue
            m = PHP_ASSIGNMENT_REGEX.match(line)
            if m:
                    ...
                    if is_candidate_secret(var_name, token):
                        findings.append(
                            SecretFinding(
                                ...
                                description="High-entropy credential assigned in PHP code",
                                masked_snippet=mask_secret(token),
                                rule_id="php_assignment_secret",
                                var_name=var_name,
                                raw_token=token,
                            )
                        )
    ...
    def redact(self, content: str, file_path: str = "") -> Tuple[str, List[SecretFinding]]:
        ...
        redacted = PHP_ASSIGNMENT_REGEX.sub(_repl, content)
