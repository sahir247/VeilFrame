"""
veilframe.folder.database — SQLite result repository for high-performance querying and persistence.

Supports fast indexed queries, multi-field search, tree reconstruction, and duplicate grouping
across millions of files without overflowing GUI memory.
"""
...
class FolderDatabase:
    """SQLite database abstraction for folder scan results."""
    ...
    def insert_files(self, files: List[FileRecord]) -> None:
        ...
        query = """
            INSERT INTO files (
                id, folder_id, name, path, relative_path, extension,
                size, created, modified, accessed, permissions,
                hash_value, hash_algorithm, quick_fingerprint, is_hidden, is_symlink, error
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = [
            (
                f.id, f.folder_id, f.name, f.path, f.relative_path, f.extension,
                f.size, f.created, f.modified, f.accessed, f.permissions,
                f.hash_value, f.hash_algorithm, f.quick_fingerprint,
                1 if f.is_hidden else 0, 1 if f.is_symlink else 0, f.error
            )
            for f in files
        ]
        with self._connect() as conn:
    ...
    def update_file_hashes(self, file_updates: List[Tuple[int, Optional[str], Optional[str], Optional[str]]]) -> None:
        """Update hash values for file records: list of (file_id, hash_value, hash_algo, error)."""
        if not file_updates:
            return
        query = """
            UPDATE files
    ...
    def update_folder_aggregates(self, folder_updates: List[FolderRecord]) -> None:
        """Update folder aggregate totals (total_files, total_folders, total_size)."""
    ...
    def insert_errors(self, errors: List[ScanError]) -> None:
        ...
        if not errors:
            return
        query = """
            INSERT INTO scan_errors (path, error_type, message, timestamp)
            VALUES (?, ?, ?, ?)
        """
        params = [(e.path, e.error_type, e.message, e.timestamp) for e in errors]
        with self._connect() as conn:
            conn.executemany(query, params)
    ...
    def search_files(
        ...
        """Fast indexed search across file metadata."""
        conditions = []
        params: List[Any] = []
    ...
    def get_extension_stats(self) -> List[ExtensionStat]:
        """Aggregate file counts and byte sizes by extension."""
        sql = """
            SELECT
    ...
    def get_largest_files(self, limit: int = 10) -> List[FileRecord]:
        """Retrieve top largest files."""
        sql = "SELECT * FROM files ORDER BY size DESC LIMIT ?"
        with self._connect() as conn:
            cursor = conn.execute(sql, (limit,))
            return [self._row_to_file(r) for r in cursor.fetchall()]

    def get_smallest_files(self, limit: int = 10) -> List[FileRecord]:
        """Retrieve top smallest non-empty files."""
        sql = "SELECT * FROM files WHERE size > 0 ORDER BY size ASC LIMIT ?"
        with self._connect() as conn:
            cursor = conn.execute(sql, (limit,))
            return [self._row_to_file(r) for r in cursor.fetchall()]

    def get_duplicate_groups(self) -> List[DuplicateGroup]:
        """Retrieve duplicate groups by exact cryptographic hash."""
        sql = """
