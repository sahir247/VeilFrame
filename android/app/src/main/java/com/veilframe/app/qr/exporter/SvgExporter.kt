 ...
 *
 * Generates true vector markup (<svg>, <rect>, <circle>, <polygon>, <defs>, <linearGradient>, <image>)
 ...
 * - Pill, Diamond, Hex, Squircle module shapes
...
object SvgExporter {
    ...
    fun generateSvg(matrix: QrMatrix, design: QrDesign): String {
        ...
        val timingHex = design.timingColor?.let { hexColor(it) }
        val alignmentHex = design.alignmentColor?.let { hexColor(it) }
        ...
        if (hasGradient) {
            sb.append("  <defs>\n")
            if (design.palette.gradientType == GradientType.RADIAL) {
        ...
        for (col in 0 until matrix.size) {
            for (row in 0 until matrix.size) {
                ...
                val fill = when {
                    type == QrMatrix.ModuleType.TIMING && timingHex != null -> timingHex
                    (type == QrMatrix.ModuleType.ALIGN_CENTER || type == QrMatrix.ModuleType.ALIGN_OTHER) && alignmentHex != null -> alignmentHex
                    else -> dataFill
                }
