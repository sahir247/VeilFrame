...
| 🏛️ **Architecture / CGI** | DAT / DAT2 | 4× | Structural geometry | 14.5M | ~30 MB | **Verified** | **Tier B** | MIT | Dual-Aggregation Transformer; excels at parallel lines, grids, brickwork, and thin railings. |
...
```text
     ...
     Cropped Face ROI            Background & Body
             │                           │
             ▼                           ▼
     Face Restoration Stage      Background Super-Resolution
...
### Qualcomm Snapdragon (NPU / GPU via QNN)
- Snapdragon 8 Gen 2, 8 Gen 3, and 8 Gen 5 processors feature the Hexagon Tensor Processor (HTP).
- **Execution Provider**: `QNN Execution Provider` (`libQnnHtp.so` / `libQnnGpu.so`).
- **Precision**: FP16 / INT8.
...
### CPU Fallback
- Constrain intra-op thread pool to **2 threads** (`setIntraOpNumThreads(2)`).
- Never peg all CPU cores simultaneously, preserving responsive Android UI thread scheduling and avoiding device thermal throttling.
