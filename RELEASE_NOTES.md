...
- **Intentional Native Android FFmpegKit Backend:** Completely eliminates Android `subprocess.run(["ffmpeg"])` `FileNotFoundError` failures by designating native `FFmpegKit` as the direct, primary video engine on Android, preserving Python exclusively for image manipulation and desktop processing.
...
- **Target File Size Quality Optimization Contract:** Maximizes visual quality subject to $\text{actual\_size} \le \text{target\_size}$ without over-compression. For PNG, evaluates compression levels 1–9 with optimization, providing clear feedback if the target ceiling is unachievable without lossy conversion.
